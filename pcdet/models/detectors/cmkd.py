...
class CMKD(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.distance_loss_weight = model_cfg.get('DISTANCE_LOSS_WEIGHT', 32)
        self.angle_loss_weight = model_cfg.get('ANGLE_LOSS_WEIGHT', 32)
    ...
    def distance_loss_func(self, bev_lidar, bev_img, bev_loss_mask):
        with torch.no_grad():
            t_d = self.pdist(bev_lidar.view(bev_lidar.shape[0], -1))
            mean_td = t_d[t_d > 0].mean()
            t_d = t_d / mean_td
        s_d = self.pdist(bev_img.view(bev_img.shape[0], -1))
        mean_sd = s_d[s_d > 0].mean()
        s_d = s_d / mean_sd

        loss_d = F.smooth_l1_loss(t_d, s_d)
        return (loss_d * bev_loss_mask).mean()
    ...
    def pdist(self, e, squared=False, eps=1e-12):
        ...
        return res.view(e.shape[0], -1)

    def angle_loss_func(self, bev_lidar, bev_img, bev_loss_mask):
        with torch.no_grad():
            td = (bev_lidar.view(bev_lidar.shape[0], -1) - bev_lidar.view(bev_lidar.shape[0], -1).unsqueeze(1))
            norm_td = F.normalize(td, p=2, dim=2)
            t_angle = torch.bmm(norm_td, norm_td.transpose(1, 2)).view(-1)

        sd = (bev_img.view(bev_img.shape[0], -1) - bev_img.view(bev_img.shape[0], -1).unsqueeze(1))
        norm_sd = F.normalize(sd, p=2, dim=2)
        s_angle = torch.bmm(norm_sd, norm_sd.transpose(1, 2)).view(-1)

        loss_a = F.smooth_l1_loss(s_angle, t_angle)
        return (loss_a * bev_loss_mask).mean()
    ...
    def get_training_loss(self, batch_dict):
        ...
        bev_distance_loss = torch.tensor(0)
        bev_angle_loss = torch.tensor(0)
        ...
        if (bev_img is not None) and (bev_lidar is not None) and self.calculate_bev_loss:
            ...
            if self.distance_loss_weight > 0:
                bev_distance_loss = self.distance_loss_func(bev_img, bev_lidar, bev_loss_mask)
            if self.angle_loss_weight > 0:
                bev_angle_loss = self.angle_loss_func(bev_img, bev_lidar, bev_loss_mask)

            bev_distance_loss *= self.distance_loss_weight
            bev_angle_loss *= self.angle_loss_weight
            loss_bev *= self.bev_loss_weight

        #all loss
        loss = loss_bev + bev_distance_loss + bev_angle_loss + loss_rpn

        tb_dict.update({ 
            "bev_loss":loss_bev.item(),
            "bev_distance_loss": bev_distance_loss.item(),
            "bev_angle_loss": bev_angle_loss.item()
        })
